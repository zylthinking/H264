...
generic_option:
    ...
    if( !tcfile_name && input_opt.timebase )
    {
        ...
        int ret = sscanf( input_opt.timebase, "%I64d/%I64d", &i_user_timebase_num, &i_user_timebase_den );
...
static int  Encode_frame( x264_t *h, hnd_t hout, x264_picture_t *pic, int64_t *last_dts )
{
    ...
    if( i_frame_size )
    {
        for(int i = 0; i < i_nal; ++i){
            output.write_frame( hout, nal[i].p_payload, nal[i].i_payload, &pic_out );
        }
...
#include "x264dll.h"
#pragma comment(lib, "x264dll.lib")
static uint8_t sei_layout[] = {
    0x09, // 0, video
    0x00, 0x00, 0x00, // 1, length
    0x00, 0x00, 0x00, 0x00, // 4, time stamp
    0x00, 0x00, 0x00, // 8, stream id
    0x00, // 11, frame type & codec
    0x01, // 12, avc nalu
    0x00, 0x00, 0x00 // 13, composition time offset
};

static uint8_t paramset_layout[] = {
    0x09, // 0, video
    0x00, 0x00, 0x00, // 1, length
    0x00, 0x00, 0x00, 0x00, // 4, time stamp
    0x00, 0x00, 0x00, // 8, stream id
    0x17, // 11, frame type & codec
    0x00, // 12, avc sequence header
    0x00, 0x00, 0x00, // 13, composition time offset
    0x01, // 16, version
    0x00, 0x00, // 17, profile
    0x00, // 19, level
    0xff, // 20, 6 bits reserved (111111) + 2 bits nal size length - 1 (11)
};

#define paramset_flv_fields     sizeof(paramset_layout) + 10
#define x264_put_byte(pch, x)   *pch++ = (uint8_t)(x & 0xff)
#define x264_put_be16(pch, x)   x264_put_byte(pch, (x >> 8)); x264_put_byte(pch, x)
#define x264_put_be24(pch, x)   x264_put_be16(pch, (x >> 8)); x264_put_byte(pch, x)
#define x264_put_be32(pch, x)   x264_put_be16(pch, (x >> 16)); x264_put_be16(pch, (x & 0xffff))

static char buf[1024 * 1024];
uint8_t* m_paramset_ptr = NULL;
uint8_t* m_sei_ptr = NULL;
uint8_t* m_slice_ptr = NULL;

int FlvPacket(void* in, BYTE** pOutBuf){
    if(m_paramset_ptr == NULL){
        m_slice_ptr = m_sei_ptr = m_paramset_ptr = (uint8_t *) &buf[0];
    }
    assert(m_sei_ptr != NULL && m_slice_ptr != NULL);

    x264_rawpic* pic = (x264_rawpic *) in;
    x264_nal* nal_ptr = pic->o_nal;
    int nr_nal = pic->o_nr_nal;
    assert(nr_nal > 0);
    ...
    for(int i = 0; i < nr_nal; ++i){
        if(nal_ptr[i].i_type == nal_sps){
            sps_index = i;
            paramset_len += nal_ptr[i].i_payload - 4;
        }else if(nal_ptr[i].i_type == nal_pps){
            pps_index = i;
            paramset_len += nal_ptr[i].i_payload - 4;
        }else if(nal_ptr[i].i_type == nal_sei){
            sei_index = i;
            sei_len += nal_ptr[i].i_payload;
        }else{
    ...
    uint8_t* p;
    if(paramset_len > 0){
        p = m_paramset_ptr + paramset_len + paramset_flv_fields;
    }else{
        p = m_sei_ptr;
    }
    ...
    int len;
    if(sei_len != 0){
        memcpy(p, &sei_layout[0], sizeof(sei_layout));
        memcpy(p + sizeof(sei_layout), nal_ptr[sei_index].p_payload, sei_len);
        sei_len += sizeof(sei_layout);
        m_sei_ptr = p;
        m_slice_ptr = m_sei_ptr + sei_len;
    }else if(p != m_sei_ptr){
        len = m_slice_ptr - m_sei_ptr;
        if(len > 0)
            memmove(p, m_sei_ptr, len);
        m_sei_ptr = p;
        m_slice_ptr = m_sei_ptr + len;
    }

    if(paramset_len != 0){
        paramset_len += paramset_flv_fields;
        memcpy(m_paramset_ptr, &paramset_layout[0], sizeof(paramset_layout));
        p = m_paramset_ptr + sizeof(paramset_layout);

        if(sps_index != -1){
            uint8_t* sps = nal_ptr[sps_index].p_payload + 4;
            uint8_t* addr = &m_paramset_ptr[17];
            x264_put_byte(addr, sps[1]);
            x264_put_byte(addr, sps[2]);
            x264_put_byte(addr, sps[3]);

            len = nal_ptr[sps_index].i_payload - 4;
            x264_put_byte(p, 0xe1);
            x264_put_be16(p, len);

            memcpy(p, sps, len);
            p += len;
        }else{
            x264_put_byte(p, 0xe0);
        }

        if(pps_index != -1){
            x264_put_byte(p, 0x01);

            len = nal_ptr[pps_index].i_payload - 4;
            x264_put_be16(p, len);
            memcpy(p, nal_ptr[pps_index].p_payload + 4, len);
            p += len;
        }else{
            x264_put_byte(p, 0x00);
        }

        len = p - m_paramset_ptr - 11;
        uint8_t* addr = &m_paramset_ptr[1];
        x264_put_be24(addr, len);
        x264_put_be32(p, (len + 11));
    }
    ...
    int64_t dts = pic->o_dts;
    int64_t cts = pic->io_pts;
    int64_t offset = cts - dts;
    uint8_t* video_start;


    if((pic->o_keyfrm) && m_sei_ptr != m_slice_ptr){
        memcpy(m_slice_ptr, nal_ptr[slice_index].p_payload, nal_ptr[slice_index].i_payload);
        p = m_slice_ptr + nal_ptr[slice_index].i_payload;
        video_start = m_sei_ptr;
    }else{
        memcpy(m_slice_ptr, &sei_layout[0], sizeof(sei_layout));
        memcpy(m_slice_ptr + sizeof(sei_layout), nal_ptr[slice_index].p_payload, nal_ptr[slice_index].i_payload);
        p = m_slice_ptr + sizeof(sei_layout) + nal_ptr[slice_index].i_payload;
        video_start = m_slice_ptr;
    }

    len = p - video_start - 11;
    uint8_t* addr = &video_start[1];
    x264_put_be24(addr, len);

    addr = &video_start[4]; 
    x264_put_be24(addr, dts);
    x264_put_byte(addr, dts >> 24);

    addr = &video_start[13];
    x264_put_be24(addr, offset);
    x264_put_be32(p, (len + 11));

    addr = &video_start[11]; 
    if(pic->o_keyfrm){
        x264_put_byte(addr, 0x17);
        *pOutBuf = m_paramset_ptr;
        return (int) (p - m_paramset_ptr);
    }

    x264_put_byte(addr, 0x27);
    *pOutBuf = video_start;
    return (int) (p - video_start);
}

int X264CalcFlvBufferSize(){
    const int sei_len = 26;
    const int sps_len = 28 - 4;
    const int pps_len = 10 - 4;
    const int paramset_len = sizeof(paramset_layout) + 10; // sps_len + sps_count + pps_count + pps_len + flv_tag_length
    const int frm_len = sizeof(sei_layout) + 4; // flv_tag_length

    return (sei_len + sps_len + pps_len + paramset_len + frm_len);
}
#endif
...
#   define  MAX_PTS_WARNING 3 /* arbitrary */
    ...
    FAIL_IF_ERROR( ticks_per_frame < 1, "ticks_per_frame invalid: %I64d\n", ticks_per_frame )

#ifdef zyl
    FILE* flvfd = NULL;
    if(flvfd == NULL){
        flvfd = fopen("e:\\h.flv", "wb");
        const BYTE hdrflvout[] = {0x46,0x4C,0x56,0x01,0x01,0x00,0x00,0x00,0x09,0x00,0x00,0x00,0x00};
        fwrite(hdrflvout, sizeof(hdrflvout), 1, flvfd);
    }

    long g_handle = x264_open(352, 288, 25, 400, 250, 25, 1000);
    x264_rawpic out;
    long n = x264_enc_header(g_handle, &out);
    if(n > 0){
        BYTE* flv;
        n = FlvPacket(&out, &flv);
        if(n > 0){
            fwrite(flv, n, 1, flvfd);
        }
    }
#endif
    ...
    for( i_frame = 0, i_frame_output = 0; !b_ctrl_c && (i_frame < param->i_frame_total || !param->i_frame_total); i_frame++ )
    {
...
#ifdef zyl
        x264_rawpic rp, out;
        rp.i_csp = cli_pic.img.csp;
        rp.i_planes = cli_pic.img.planes;
        rp.io_pts = (GetTickCount() - tick) * 25 / 1000;
        memcpy(&rp.i_stride, &cli_pic.img.stride, sizeof(cli_pic.img.stride));
        memcpy(&rp.i_plane, &cli_pic.img.plane, sizeof(cli_pic.img.plane));

        long ret = x264_encode(g_handle, &rp, &out);


        if(ret > 0){
            static FILE* file = fopen("e:\\frame.txt", "w");
            fprintf(file, "%d\n", out.o_type);

            BYTE* flv;
            int flv_len = FlvPacket(&out, &flv);
            if(flv_len > 0){
                fwrite(flv, flv_len, 1, flvfd);
            }
        }
        ...
        if( pic.i_pts <= largest_pts )
        {
            if( cli_log_level >= X264_LOG_DEBUG || pts_warning_cnt < MAX_PTS_WARNING )
                x264_cli_log( "x264", X264_LOG_WARNING, "non-strictly-monotonic pts at frame %d (%I64d <= %I64d)\n",
