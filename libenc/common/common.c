 ...
 * x264_param_default:
 ****************************************************************************/
x264_api void x264_param_default( x264_param_t *param )
{
    /* */
...
x264_api int x264_param_parse( x264_param_t *p, const char *name, const char *value )
{
    char *name_buf = NULL;
    int b_error = 0;
    int name_was_bool;
...
x264_api int x264_picture_alloc( x264_picture_t *pic, int i_csp, int i_width, int i_height )
{
    ...
    pic->img.plane[0] = (uint8_t *)x264_malloc( 3 * i_width * i_height / 2 );
    if( !pic->img.plane[0] )
        return -1;
...
#else
    uint8_t *buf = (uint8_t *)malloc( i_size + 15 + sizeof(void **) );
    if( buf )
    {
