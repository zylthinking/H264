...
static void x264_frame_dump( x264_t *h )
{
    ...
    pixel *planeu = (pixel *)x264_malloc( cw*ch*2*sizeof(pixel) );
...
x264_api x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    if( h->param.b_dts_compress )
    {
        ...
        if( h->i_dts_compress_multiplier != 1 )
        {
            ...
            x264_log( h, X264_LOG_DEBUG, "DTS compression changed timebase: %u/%u -> %u/%I64d\n",
    ...
    {
        x264_log( h, X264_LOG_ERROR, "Effective timebase denominator %I64d exceeds H.264 maximum\n", new_timebase_den );
        goto fail;
    }
 ...
 ****************************************************************************/
x264_api int x264_encoder_reconfig( x264_t *h, x264_param_t *param )
{
    int rc_reconfig = 0;
 ...
 ****************************************************************************/
x264_api void x264_encoder_parameters( x264_t *h, x264_param_t *param )
{
...
static int x264_encoder_encapsulate_nals( x264_t *h, int start )
{
    ...
    {
        uint8_t *buf = (uint8_t *)x264_malloc( nal_size * 2 + h->out.i_nal * 4 );
        if( !buf )
            return -1;
 ...
 ****************************************************************************/
x264_api int x264_encoder_headers( x264_t *h, x264_nal_t **pp_nal, int *pi_nal )
{
    int frame_size = 0;
...
x264_api void x264_encoder_intra_refresh( x264_t *h )
{
    h = h->thread[h->i_thread_phase];
    h->b_queued_intra_refresh = 1;
}

x264_api int x264_encoder_invalidate_reference( x264_t *h, int64_t pts )
{
    if( h->param.i_bframe )
    {
 ...
 *       B      6   2*5
 ****************************************************************************/
x264_api int     x264_encoder_encode( x264_t *h,
                             x264_nal_t **pp_nal, int *pi_nal,
 ...
 * x264_encoder_close:
 ****************************************************************************/
x264_api void    x264_encoder_close  ( x264_t *h )
{
 ...
 ****************************************************************************/
x264_api int x264_encoder_delayed_frames( x264_t *h )
{
