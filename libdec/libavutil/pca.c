...
int ff_pca(PCA *pca, double *eigenvector, double *eigenvalue){
    ...
    int k=0;
    const int n= pca->n;
    double *pz = (double*) av_mallocz(n * sizeof(double));

    memset(eigenvector, 0, sizeof(double)*n*n);
	memset(pz, 0, sizeof(double)*n);
        ...
        for(i=0; i<n; i++){
            for(j=i+1; j<n; j++){
                ...
                if(fabs(covar) == 0.0) //FIXME should not be needed
                    continue;
                if(pass >=3 && fabs((eigenvalue[j]+pz[j])/covar) > (1LL<<32) && fabs((eigenvalue[i]+pz[i])/covar) > (1LL<<32)){
                    pca->covariance[j + i*n]=0.0;
                    continue;
                }

                h= (eigenvalue[j]+pz[j]) - (eigenvalue[i]+pz[i]);
                theta=0.5*h/covar;
