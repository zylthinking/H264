...
static inline int ls_get_code_regular(GetBitContext *gb, JLSState *state, int Q){
    ...
    if(!state->near1 && !k && (2 * state->B[Q] <= -state->N[Q]))
        ret = -(ret + 1);
...
static inline void ls_decode_line(JLSState *state, MJpegDecodeContext *s, void *last, void *dst, int last2, int w, int stride, int comp, int bits){
    ...
    while(x < w) {
        ...
        D2 = Rc - Ra;
        /* run mode */
        if((FFABS(D0) <= state->near1) && (FFABS(D1) <= state->near1) && (FFABS(D2) <= state->near1)) {
            int r;
            int RItype;
        ...
        if(state->near1){
            ...
            else if(pred > state->maxval + state->near1)
                pred -= state->range * state->twonear;
...
int ff_jpegls_decode_picture(MJpegDecodeContext *s, int near1, int point_transform, int ilv){
    int i, t = 0;
