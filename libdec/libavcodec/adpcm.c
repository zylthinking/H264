...
#define FREEZE_INTERVAL 128
   ...
   TrellisPath paths[max_paths];
   TrellisNode node_buf[2][frontier];
   TrellisNode *nodep_buf[2][frontier];
#else
   TrellisPath *paths = _alloca(max_paths * sizeof(TrellisPath));
   TrellisNode **node_buf = _alloca(2*frontier*sizeof(TrellisNode));
   TrellisNode ***nodep_buf = _alloca(2*frontier*sizeof(TrellisNode *)); 
#endif
...
/*                c->status[1].step_index = 0; */
            ...
            if(avctx->trellis > 0) {
                ...
                uint8_t buf[2][n*8];
#else
				uint8_t **buf = _alloca(2*n*8 * sizeof(uint8_t));
#endif
    ...
    case CODEC_ID_ADPCM_SWF:
    {
        ...
        if(avctx->trellis > 0) {
            ...
            uint8_t buf[2][n];
#else
	    	uint8_t **buf = _alloca(2* n * sizeof(uint8_t));
#endif
    ...
    case CODEC_ID_ADPCM_YAMAHA:
        ...
        if(avctx->trellis > 0) {
            ...
            uint8_t buf[2][n*2];
#else
	    	uint8_t **buf = _alloca(2* n * 2 * sizeof(uint8_t));
#endif
            n *= 2;
            if(avctx->channels == 1) {
                adpcm_compress_trellis(avctx, samples, buf[0], &c->status[0], n);
...
#if CONFIG_ENCODERS
const enum SampleFormat tmp_sample_fmts[] = {SAMPLE_FMT_S16,SAMPLE_FMT_NONE};
